...
use ippi::{Config, dht, p2p, tunnel};
use std::net::IpAddr;
use std::str::FromStr;
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tunnel_id = tunnel_manager
        .create_tunnel("ippi-tun0", tunnel::TunnelType::Tun, local_ip)
        .await?;
    ...
    if let Some(tunnel_info) = tunnel_manager.get_tunnel(&tunnel_id).await? {
        ...
        println!(
            "   Estado: {:?}, MTU: {}",
            tunnel_info.status, tunnel_info.mtu
        );
    }
    ...
    dht_manager
        .add_peer("QmPeer1", "/ip4/192.168.1.100/tcp/4001")
        .await?;
    dht_manager
        .add_peer("QmPeer2", "/ip4/192.168.1.101/tcp/4001")
        .await?;
    dht_manager
        .add_peer("QmPeer3", "/ip4/192.168.1.102/tcp/4001")
        .await?;
