...
impl P2pManager {
        ...
        #[cfg(feature = "p2p")]
        {
            match self.initialize_libp2p().await {
                Ok((peer_id, addrs)) => {
                    ...
                    tracing::info!("P2P initialized with peer ID: {}", state.peer_id.as_ref().unwrap());
                }
                Err(e) => {
    ...
    pub async fn send_message(&self, to_peer_id: &str, topic: &str, payload: &[u8]) -> Result<()> {
        ...
        #[cfg(not(feature = "p2p"))]
        {
            tracing::debug!("Simulating P2P message to {} on topic {}: {} bytes", 
                to_peer_id, topic, payload.len());
        }
    ...
    pub async fn broadcast_message(&self, topic: &str, payload: &[u8]) -> Result<usize> {
        ...
        {
            let peers = self.peers.read().await;
            tracing::debug!("Simulating broadcast to {} peers on topic {}: {} bytes", 
                peers.len(), topic, payload.len());
            Ok(peers.len())
        }
    }
...
mod network {
    ...
    impl P2pManager {
        ...
        async fn send_libp2p_message(&self, _to_peer_id: &str, _topic: &str, _payload: &[u8]) -> Result<()> {
