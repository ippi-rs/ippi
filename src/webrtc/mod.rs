...
impl WebRtcManager {
    pub fn new(config: Arc<Config>) -> Result<Self> {
        let state = WebRtcState {
            ...
            initialized: false,
            stun_servers: config.webrtc.as_ref()
                .map(|w| w.stun_servers.clone())
                .unwrap_or_default(),
            turn_servers: config.webrtc.as_ref()
                .map(|w| w.turn_servers.clone())
                .unwrap_or_default(),
            video_bitrate: config.webrtc.as_ref()
                .map(|w| w.video_bitrate)
                .unwrap_or(2000),
            audio_bitrate: config.webrtc.as_ref()
        ...
        #[cfg(feature = "webrtc")]
        {
            match self.initialize_webrtc().await {
                Ok(_) => {
                    ...
                    tracing::info!("WebRTC initialized with {} STUN servers, {} TURN servers", 
                        state.stun_servers.len(), state.turn_servers.len());
                }
                Err(e) => {
    ...
    pub async fn create_video_stream(&self, session_id: &str, width: u32, height: u32, framerate: u32) -> Result<String> {
        let mut sessions = self.sessions.write().await;
        let mut state = self.state.write().await;
        ...
        tracing::info!("Created video stream {} for session {} ({}x{} @ {}fps)", 
            stream_id, session_id, width, height, framerate);
    ...
    pub async fn create_audio_stream(&self, session_id: &str) -> Result<String> {
        ...
        tracing::info!("Created audio stream {} for session {}", stream_id, session_id);
    ...
    pub async fn close_session(&self, session_id: &str) -> Result<()> {
        ...
        let stream_ids: Vec<String> = streams.iter()
...
mod rtc {
    ...
    impl WebRtcManager {
        ...
        async fn add_webrtc_ice_candidate(&self, _session_id: &str, _candidate: IceCandidate) -> Result<()> {
