...
impl KvmManager {
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        #[cfg(feature = "kvm")]
        {
            let device_path = self.config.kvm.as_ref()
            ...
            if !Path::new(device_path).exists() {
                ...
                state.enabled = false;
                return Err(Error::Kvm(format!("KVM device not found at {}", device_path)));
            }
    ...
    pub async fn create_vm(&self, name: &str, memory_mb: u64, vcpus: u32) -> Result<String> {
        ...
        tracing::info!("Created VM {} (ID: {}) with {}MB RAM, {} vCPUs", name, id, memory_mb, vcpus);
...
mod vm {
    ...
    impl VmHandle {
        pub fn new(memory_mb: u64) -> Result<Self> {
            ...
            let vm_fd = kvm.create_vm().map_err(|e| Error::Kvm(format!("Failed to create VM: {}", e)))?;
